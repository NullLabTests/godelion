...
class EvaluationError(Exception):
    ...
    def __str__(self):
        return f"Evaluation error for {self.instance_id}: {self.super_str}\nCheck ({self.log_file}) for more information."
...
def get_dataset_from_preds(dataset_name: str, split: str, instance_ids: list, predictions: dict, run_id: str, exclude_completed: bool = True):
    """
    ...
    for instance in dataset:
        ...
        report_file = RUN_EVALUATION_LOG_DIR / run_id / prediction["model_name_or_path"].replace("/", "__") / prediction[KEY_INSTANCE_ID] / "report.json"
...
def make_run_report(predictions: dict, full_dataset: list, client: docker.DockerClient, run_id: str) -> Path:
    ...
    report_file = Path(list(predictions.values())[0]["model_name_or_path"].replace("/", "__") + f".{run_id}" + ".json")

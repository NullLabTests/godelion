...
def get_response_withtools(client, model, messages, tools, tool_choice, logging=None, max_retry=3):
    try:
        if "claude" in model:
            ...
            response = client.responses.create(model=model, input=messages, tool_choice=tool_choice, tools=tools, parallel_tool_calls=False)
            response = response
        else:
...
def convert_block_claude(block):
    ...
    if block_type == "text":
        ...
        return {"type": "text", "text": f"<tool_use>\n{{'tool_name': {tool_name}, 'tool_input': {tool_input}}}\n</tool_use>"}
